...
7. **Testability & unit tests.** Use **Vitest** (Vite-native — reuses the app's own Vite config/transform, faster than Jest, zero extra setup) with **React Testing Library** + **jsdom**. Components are tested against a **stubbed `api_client`** (the single seam to the backend) or a spec-driven mock of `openapi.yaml` — never a live backend. Supabase Realtime is mocked by invoking the `onFeatureInsert`/`onBroadcastEvent`/`onDeployment` callbacks directly. The frozen `styles.css` / Design Lock is **not** unit-tested — that's the separate screenshot-diff concern in Rule 6. No unit test hits a live backend or Realtime.